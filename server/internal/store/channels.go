...
func scanModel(s scanner) (Model, error) {
	...
	if mcpServerIDs.Valid {
		m.MCPServerIDs = json.RawMessage(mcpServerIDs.String)
		if normalized, err := NormalizeMCPServerIDs(m.MCPServerIDs); err == nil {
			m.MCPServerIDs = normalized
		}
	}
...
func CreateModel(ctx context.Context, db *sql.DB, m Model) (*Model, error) {
	...
	if mcpServerIDs, err := NormalizeMCPServerIDs(m.MCPServerIDs); err != nil {
		return nil, err
	} else {
...
// nullable model-tool policies rely on that distinction: NULL defaults to all
// eligible tools/services, while [] explicitly selects none.
