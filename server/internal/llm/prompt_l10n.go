...
var promptL10nTable = map[string]promptL10n{
	"en": {
		...
		toolCite:           "- Cite your sources: when you use a web_search result, place its [n] marker inline right after the claim it supports.\n",
	...
	"zh": {
		...
		toolCite:           "- 标注来源：使用 web_search 的结果时，在其支撑的说法紧后放置该结果的 [n] 标记进行行内引用。\n",
	...
	"zh-Hant": {
		...
		toolCite:           "- 標註來源：使用 web_search 的結果時，在其支撐的說法緊後放置該結果的 [n] 標記進行行內引用。\n",
	...
	"ja": {
		...
		toolCite:           "- 出典を示す：web_search の結果を使うときは、その主張の直後に結果の [n] マーカーをインラインで置いてください。\n",
	...
	"fr": {
		...
		toolCite:           "- Cite tes sources : quand tu utilises un résultat de web_search, place son marqueur [n] en ligne juste après l'affirmation qu'il appuie.\n",
