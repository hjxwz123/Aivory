...
| **AI PPT 创作台** | 管理员接入文多多 Docmee API、设定每份 PPT 的积分价格后，登录用户即可安全地创建演示文稿，每次生成与扣费都会记录在用量报表中。 |
...
可选的 **AI PPT** 创作台在「**管理后台 → 积分与配额 → AI PPT（文多多 Docmee）**」中配置。管理员保存 Docmee API Key 并打开启用开关后，登录用户的侧边栏才会显示 **AI PPT**；未配置或已关闭时入口保持隐藏，`/ppt` 页面也不能发起生成。生成按「每份固定积分」计费（与对话共用同一套积分开关），每次生成与付费编辑都会出现在「**管理后台 → 用量与计费**」。配置步骤、积分计费与模板管理请阅读[《AI PPT（文多多 Docmee）使用与配置》](docs/ai-ppt-docmee.md)。
