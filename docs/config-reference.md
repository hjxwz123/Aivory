...
| `AIVORY_LLM_INLINE_COMPACTION_BACKLOG_FACTOR` | `int` | `3` | `llm/compaction.go:67` | Multiplier on keepRounds*2 for the un-summarised tail length that forces inline (rather than async) compaction; the planner also preserves one extra batch above the asynchronous high watermark. |
